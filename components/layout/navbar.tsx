...
export default function Navbar() {
  ...
  // Read auth status on mount
  useEffect(() => {
    const providerLoggedIn = localStorage.getItem("isLoggedIn") === "true";
    ...
    if (providerLoggedIn) {
      ...
      if (currentUser) {
        try {
          ...
          setUserName(user.businessName || user.ownerName || "Provider");
        } catch (e) {}
      }
      ...
      if (currentCustomer) {
        try {
          ...
          setUserName(customer.name || customer.email || "Customer");
        } catch (e) {}
      }
    } else {
      setIsLoggedIn(false);
  ...
  }, [pathname]); // Check auth when navigation occurs

  // Sync searchVal with URL search query param
  useEffect(() => {
  ...
  // Build dynamic desktop nav links
  const navLinks = [
  ...
  if (isLoggedIn) {
    ...
    navLinks.push({ href: dashboardUrl, label: "Dashboard" });
  }

  // Build dynamic mobile sidebar sections
        ...
        <div className="container mx-auto px-4">
          <div className="flex items-center justify-between h-16">
                ...
                <>
                  <span
                    className={`mx-2 h-4 w-px ${
                      showSolid ? "bg-slate-200" : "bg-white/20"
                    }`}
                  />
                  <span
            ...
            >
              <Menu
                size={22}
                className={showSolid ? "text-slate-700" : "text-white"}
              />
            </button>
          </div>
        </div>
